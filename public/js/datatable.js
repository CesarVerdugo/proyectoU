...
$(document).ready(function() {
  ...
  role_permissionList();
      ...
      data_editcategory("#category tbody", tablecategory);
      data_showcategory("#category tbody", tablecategory);
      data_deletecategory("#category tbody", tablecategory);
      }


      //Estrae la información de la tabla para editar.

      var data_editcategory =  function(tbody , tablecategory){

      $(tbody).on("click", "#editCategory", function(){

        var data= tablecategory.row($(this).parents("tr")).data();
        var idcategory =$("#idcategoryedit").val(data.id);

        var name=$("#namecategoryedit").val(data.name);
      ...
      var data_showcategory =  function(tbody , tablecotizacion){

      $(tbody).on("click", "#showCategory", function(){

        var data= tablecategory.row($(this).parents("tr")).data();
            var id =$("#showidcategory").val(data.id);
            var  name =$("#shownamecategory").val(data.name);
      ...
      // Estrae el id de la categoria de una columan especifica para eliminar

      var data_deletecategory =  function(tbody , tablecategory){
...
$(tbody).on("click", "#deleteProduct", function(){
  ...
  var idproduct =$("#idproductedit").val(data.id);
 ...
 $(tbody).on("click", "#editProduct", function(){
    var data= tableProduct.row($(this).parents("tr")).data();
    var category_id=$("#category_id").val(data.category_id);
    var idproductedit =$("#idproductedit").val(data.id);

    var code=$("#code").val(data.code);
    
  //  var fecha=$("#date").val(data.date);
    var name=$("#name").val(data.name);
    var stockmin=$("#stockmin").val(data.stockmin);
    var description=$("#description").val(data.description);
    var sale_price=$("#sale_price").val(data.sale_price);
    var unit_price=$("#unit_price").val(data.unit_price);
...
$(tbody).on("click", "#showPermission", function(){
  ...
  // Datatable de comentarios pagina Principal
  function commentsListPagePrincipal() {
  var tabletcomment =   $('#commentariesPagePrincipal').DataTable(
    {  language: {
              "sProcessing":     "Procesando...",
              "sLengthMenu":     "Mostrar _MENU_ registros",
              "sZeroRecords":    "No se encontraron resultados",
    ...
    data_showcomment("#commentariesPagePrincipal tbody", tabletcomment)
  }
  //Estrae la información de la tabla para ver.
